...
byte-exact (md5) before being trusted or written up, and every headline
number was independently hand-recomputed from the committed
per-patient/per-donor tables before being reported — same discipline as
every prior step.
...
Total from the two original qsub jobs: 62 files. **Round-2 review
correction**: round 1 added one further post-review artifact
(`htan_name_similarity_supplementary_check.tsv`, §"HTAN study-provenance
overlap audit" below) — a 63rd committed result artifact, generated by
a real (re-run, not simulated) lightweight check, now wired into
`dataset_extension_scoring.py`'s main driver so the full workflow
(including this supplementary check) is reproducible end-to-end from a
single script, not split across a script and an ad hoc interactive
check.
...
respectively). **Round-2 review correction**: round 1 additionally
claimed this "matches PR #27's own primary-compute precedent (which also
commits per-cell scores)" — checked directly against the repo, this is
false: PR #27's own per-cell `crc_gut_scoring_all_panels.parquet` and
`crc_gut_scoring_cell_metadata.parquet` were never committed to git
(only kept on Argos; only the small `n_testable_genes_per_panel.tsv`
summary was committed), the same "small deliverable" convention the
secondary/tertiary composition analyses also followed. Corrected: this
PR's choice to commit per-cell scores for all 3 scoring passes is a
**new decision for this PR specifically** (these extension datasets are
far smaller than the 665,473-cell primary atlas, so the size tradeoff is
different), not a continuation of an existing precedent. All output
tables committed in full
...
- **Round 1 (REQUEST_CHANGES — one genuine pre-compute contract
  ...
  36 missing genes are present in the primary atlas and 34/36 are
  present in HTAN — **round-2 review correction**: this bullet
  originally (wrongly) summarized the investigation as "all 36 present
  in both other datasets," contradicting the accurate 34/36-in-HTAN
  figure already stated correctly elsewhere in this document — fixed to
  match. Despite CRLM having the most total genes of the three datasets,
  this rules out random dropout — a genuine CRLM-specific reference/
  annotation gap for this gene family. **Round-2 review correction,
  gate mechanics**: round 1's fix still let scoring proceed regardless
  of the flag, which wasn't the design's actual "blocks compute until
  understood" contract — fixed to encode the CRLM/P_Gut-specific case as
  a named, investigated exception (exact 40/76 match required) and raise
  on any other, still-unexplained coverage deviation; the existing
  40-gene score remains usable and is kept as-is (re-confirmed against
  real data, no rescoring). (2) The
  ...
  PR does commit per-cell scores for all 3 passes — corrected in both
  this doc and the PR body.

- **Round 2 (REQUEST_CHANGES — 1 remaining contract-implementation gap
  plus 3 provenance/bookkeeping inconsistencies introduced by the round-1
  fix commit itself, all independently verified before fixing, no
  rescoring required)**: (1) round 1's coverage-gate fix still let
  scoring proceed regardless of the flag — not the design's actual
  "blocks compute until understood" contract, just a louder waiver.
  Fixed to encode the investigated CRLM/`P_Gut-specific` case as a named
  exception (exact `(40, 76)` match) and raise on any other unexplained
  deviation — re-run against real data, confirms the known case still
  passes and the mechanism would correctly block a genuinely new one.
  (2) A real self-introduced factual contradiction: this document's
  detailed investigation correctly said 34/36 missing genes are present
  in HTAN, but the round-1 review-history bullet (and the PR body)
  wrongly summarized it as "all 36 present in both other datasets" —
  re-verified directly against the source data (36/36 in the primary
  atlas, 34/36 in HTAN) and fixed all three locations to agree. (3)
  `htan_name_similarity_supplementary_check()` was defined but never
  called anywhere — the advertised scoring workflow could not actually
  regenerate its own committed output — fixed by wiring it into
  `dataset_extension_scoring.py`'s main driver; re-run independently,
  confirms byte-identical output to what was already committed. (4) The
  round-1 fix claimed committing per-cell scores here "matches PR #27's
  own primary-compute precedent" — checked directly against the repo,
  false: PR #27's per-cell files were never committed to git, only kept
  on Argos — fixed to state this is a new decision for this PR
  specifically, not a continuation of an existing precedent. File-count
  bookkeeping also updated: 62 files from the two original qsub jobs,
  63 including the now-wired-in supplementary check.
