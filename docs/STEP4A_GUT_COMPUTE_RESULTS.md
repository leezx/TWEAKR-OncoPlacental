...
## Round-2 corrections (PR #21 round-2 review, 2 more real issues in the round-1 fix itself)

1. **The gene-ID audit's own methodology was unreliable.** The round-1 fix (`gene_id_audit.py` v1) detected "duplicate-symbol uniquification" by stripping a trailing `-\d+` from `var_name` and treating string matches as anndata artifacts. This is a real methodological flaw: many genuine HGNC symbols legitimately end in a dash-number as part of their own canonical name — immunoglobulin/TCR variable-region genes like `IGHVIII-5` are exactly this case, and are exactly the gene family the v1 audit's own output flagged as "mostly Ig/TCR families" — which turned out to be the heuristic's own likely false-positive pattern, not confirmation of anything. **Fixed**: `scripts/04a_dfp_gut/gene_id_audit.py` v2 abandons the string-pattern heuristic entirely and instead compares every `var_name` against BioMart's real, authoritative `external_gene_name` for that exact `gene_id` (bulk query, `scripts/04a_dfp_gut/query_human_biomart_full.py` → `results/04a_dfp_gut/gene_id_audit/human_biomart_full.tsv`, 86,411 human genes, same bulk-query pattern already used elsewhere in this project). A `var_name` is now only flagged "renamed/mismatched" if it provably differs from the authoritative symbol for its own Ensembl ID — not based on what it looks like. Real result with the corrected method: **3,135 `var_names` are genuinely renamed/mismatched** (mostly old clone-based identifiers like `AL669831.5` that BioMart now resolves to current lincRNA names like `LINC01409` — real nomenclature drift between this h5ad's reference annotation and the current archive) and **8,116 `var_names` are not found in this BioMart snapshot at all** (reported as a separate, honest category, not conflated with "renamed" — could be retired/very old IDs). Spot-checked: `IGF2` (`ENSG00000284779`) correctly shows `found_in_biomart=False` (BioMart has no name for it, consistent with "novel protein"); `IGF2-1` (`ENSG00000167244`) correctly shows `renamed_or_mismatched=True` with `authoritative_symbol=IGF2`; `IGF2-AS` (a real, unrelated gene that happens to share the `IGF2` prefix) correctly shows `renamed_or_mismatched=False` — the v1 heuristic would have wrongly grouped `IGF2-AS` in with the `IGF2`/`IGF2-1` "duplicate" bucket; v2 does not.
2. **The `P_developmental_primary84.txt` collision check re-run with the corrected method — same bottom-line result.** Checked whether any of the 84 `P_developmental` genes match either side of a renamed pair (the h5ad's outdated `var_name`, or the authoritative symbol a renamed gene should have had): **0 collisions**, same conclusion as the (methodologically weaker) round-1 check — `D_Colon-shared`/`D_SI-shared`/`P_*-specific` set arithmetic is confirmed, via the correct method this time, to be unaffected by any gene-symbol renaming in this dataset.
