...
Picked up the reviewer's explicit next step directly. Wrote `scripts/04_dfp_signature/hpa_placenta_enrichment.py`: builds an independent "HPA placenta-enriched" reference set (82 genes) directly from `rna_tissue_hpa.tsv` using HPA's own published "Tissue enriched" definition (≥4× every other of HPA's 39 tissues, floor nTPM≥1) — independent of the scRNA-seq datasets the edgeR DE used, so a real external check rather than circular validation. Ran on Argos, results pulled back byte-exact.
